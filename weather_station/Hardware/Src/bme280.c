...
#define MAX44009_ADDRESS (0x4a << 1) // MAX44009的I2C地址
...
{					     
		...
		I2cHandle.Init.ClockSpeed      = 30000;             /* Communication speed 100K */                      /* I2C communication speed */
		I2cHandle.Init.DutyCycle       = I2C_DUTYCYCLE_16_9;                                                         /* I2C Duty cycle */
		I2cHandle.Init.OwnAddress1     = BME280_ADDRESS;                                                           /* I2C address */
...
void iicDevRead(uint8_t devaddr,uint8_t addr,uint8_t len,uint8_t *rbuf)
{
	...
	if(HAL_I2C_Mem_Read(&I2cHandle, devaddr, addr, I2C_MEMADD_SIZE_8BIT, rbuf, len,1000) != HAL_OK)
  {
    while (1);
  }
...
{				   	  	    																 
	...
	rbuf[0] = data;
	/* I2C master transmission*/
	if(HAL_I2C_Mem_Write(&I2cHandle, devaddr, addr, I2C_MEMADD_SIZE_8BIT, rbuf, 1,1000) != HAL_OK)
  {
    while (1);
  }
  /*Check the current I2C state*/
