...
{   
    ...
    uint32_t start = SysTick -> VAL; uint32_t load = SysTick -> LOAD; 
    while(1){ 
    uint32_t now = SysTick -> VAL;
    uint32_t elapsed = (start>=now) ? (start-now) : (start + (load - now)); 
...
{
    HAL_GPIO_WritePin(GPIOF, IIC_SCL, (GPIO_PinState)(x));
    delay_us(5);
}

static void AS5600_W_SDA(uint8_t x)
{    
	  HAL_GPIO_WritePin(GPIOF, IIC_SDA, (GPIO_PinState)(x));
    delay_us(5);
}
...
{
    uint8_t tmp;
    tmp = HAL_GPIO_ReadPin(GPIOF, IIC_SDA);
    delay_us(5);
    return tmp;
}
...
void AS5600_IIC_Init(void)
{
    ...
    /* 使能GPIOB时钟 */
    __HAL_RCC_GPIOF_CLK_ENABLE(); // HAL库的时钟使能宏

    /* 配置 SCL(PF1) 与 SDA(PF0) 为 GPIO 开漏输出模式 */
    GPIO_InitStruct.Pin = IIC_SDA | IIC_SCL;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_OD;       			// 开漏输出
    GPIO_InitStruct.Pull = GPIO_NOPULL;                 // 开漏输出通常外部上拉，此处不启用内部上下拉
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;     	// 高速模式 (对应50MHz)
    HAL_GPIO_Init(GPIOF, &GPIO_InitStruct);

    /* 初始化后将总线置为空闲状态（SCL和SDA均为高电平） */
    AS5600_W_SCL(1);
    AS5600_W_SDA(1);
}
...
{
    uint16_t raw = AS5600_GetRAWAngleData();
    raw &= 0x0FFF;
    uint16_t idx = ((raw + 128u) / 256u) & 0x0Fu;
    static const WindDirection map[16] = {
